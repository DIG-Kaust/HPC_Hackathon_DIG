...
class _Simplex_numba_gpu(ProxOperator):
   ...
   """
    def __init__(self, n, radius, dims=None, axis=-1,
        ...
        self.n = n
        self.coeffs = np.ones(self.n if dims is None else dims[axis]).todevice()
        self.radius = radius
        self.dims = dims
        self.axis = axis
    ...
    def prox(self, x, tau):
        ...
        simplex_jit_gpu[num_blocks,num_threads_per_blocks](x, self.coeffs, self.radius, 0, 10000000000, self.maxiter, self.ftol, self.xtol, y)

        # y = simplex_jit_gpu(x, self.coeffs, self.radius, 0, 10000000000,
